...
export const TeamProjectDashboard: React.FC<TeamProjectDashboardProps> = ({ project }) => {
  ...
  return (
    <div className="space-y-6 selection:bg-amber-500 selection:text-black">
      ...
      <div className="p-6 rounded-3xl bg-gradient-to-r from-[#12151f] via-[#0e1118] to-[#161309] border border-amber-500/25 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 shadow-2xl shadow-black">
        <div className="space-y-1">
          <div className="flex flex-wrap items-center gap-3">
            <h2 className="text-2xl font-black text-white tracking-tight">{project.name}</h2>
            <span className="px-2.5 py-0.5 rounded-full text-xs font-mono font-bold bg-amber-500/15 text-amber-300 border border-amber-500/30 flex items-center gap-1.5 capitalize">
            ...
            {isRealtimeConnected ? (
              ...
              <span className="px-2.5 py-0.5 rounded-full text-xs font-mono font-bold bg-[#141724] text-slate-400 border border-amber-500/15 flex items-center gap-1.5">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <button
            ...
            className="px-3.5 py-2.5 rounded-xl bg-[#141724] hover:bg-[#1f2436] text-amber-200 text-xs font-bold border border-amber-500/20 flex items-center gap-1.5 transition"
            title="Sync all Git/GitHub modules now"
          >
            <RefreshCw className={`w-3.5 h-3.5 text-amber-400 ${isSyncingProject ? 'animate-spin' : ''}`} />
          ...
          <button
            ...
            className="px-4 py-2.5 rounded-xl bg-gradient-to-r from-amber-400 to-yellow-500 hover:from-amber-300 hover:to-yellow-400 text-black text-xs font-extrabold shadow-lg shadow-amber-500/25 flex items-center gap-2 transition disabled:opacity-50"
          >
          ...
          <button
            ...
            className="px-4 py-2.5 rounded-xl bg-[#141724] hover:bg-[#1f2436] text-amber-200 text-xs font-bold border border-amber-500/20 flex items-center gap-2 transition"
          >
          ...
          <button
            ...
            className="px-4 py-2.5 rounded-xl bg-gradient-to-r from-amber-400 via-yellow-500 to-amber-600 hover:from-amber-300 hover:via-yellow-400 hover:to-amber-500 text-black text-xs font-extrabold shadow-lg shadow-amber-500/25 flex items-center gap-2 transition"
          >
      ...
      {/* Team Members List */}
      <div className="p-5 rounded-3xl bg-[#0e1118] border border-amber-500/20 space-y-3 shadow-xl">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 text-sm font-bold text-white">
            <Users className="w-4 h-4 text-amber-400" />
            <span>Team Members ({members.length + 1})</span>
          </div>
          <span className="text-xs text-amber-400/70 font-mono">GitHub is Source of Truth</span>
        </div>

        <div className="flex flex-wrap items-center gap-3 pt-1">
          {/* Owner Badge */}
          <div className="px-3 py-1.5 rounded-xl bg-[#08090d] border border-amber-500/30 flex items-center gap-2 text-xs">
            <div className="w-6 h-6 rounded-full bg-gradient-to-r from-amber-400 to-yellow-500 text-black font-extrabold flex items-center justify-center text-[10px]">
              S
            </div>
            <div>
              <span className="font-bold text-white block leading-tight">Shalya</span>
              <span className="text-[10px] text-amber-300 font-mono">Owner</span>
            </div>
          </div>
            ...
            return (
              <div
                ...
                className={`px-3 py-1.5 rounded-xl border flex items-center gap-2 text-xs group transition ${
                  isPending ? 'bg-[#08090d] border-amber-500/30' : 'bg-[#08090d] border-amber-500/15'
                }`}
              >
                <div className={`w-6 h-6 rounded-full text-black font-bold flex items-center justify-center text-[10px] ${
                  isPending ? 'bg-amber-400' : 'bg-yellow-500'
                }`}>
                ...
                {isPending && m.inviteToken && (
                  <button
                    ...
                    className="p-1 rounded text-slate-400 hover:text-amber-300 hover:bg-[#141724] transition ml-1"
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {project.modules.map((pm) => {
            ...
            return (
              <div key={pm.id} className="p-5 rounded-3xl bg-[#0e1118] border border-amber-500/20 space-y-4 hover:border-amber-400/40 transition shadow-xl shadow-black/80">
                {/* Module Header */}
                <div className="flex items-start justify-between">
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] font-mono font-bold text-amber-400 uppercase">{pm.module.categoryName}</span>
                      <span className="text-[10px] font-mono px-2 py-0.5 rounded-lg bg-[#08090d] text-amber-200 border border-amber-500/20">
                  ...
                  <div className="flex flex-col items-end gap-1">
                    ...
                    {isRunningLocally ? (
                      ...
                      <span className="px-2.5 py-0.5 rounded-full text-[11px] font-mono font-bold bg-[#141724] text-slate-400 border border-amber-500/15 flex items-center gap-1">
                        <span>⚪ Stopped</span>
                      </span>
                    )}
                ...
                <div className="p-3.5 rounded-2xl bg-[#08090d] border border-amber-500/15 space-y-2 font-mono text-xs">
                  ...
                  <div className="flex items-center justify-between text-slate-400">
                    ...
                    <span className="text-amber-300 font-bold">{branch}</span>
                  </div>

                  {/* Latest Synced Commit Details */}
                  <div className="pt-1.5 border-t border-amber-500/15 space-y-1">
                    <div className="flex items-center justify-between text-slate-400 text-[11px]">
                      <span>Latest Commit:</span>
                      <span className="text-yellow-400 font-bold">{commitSha ? commitSha.slice(0, 7) : 'Initial'}</span>
                    </div>
                  ...
                  {(localState?.frontendUrl || pm.deploymentUrl) && (
                    <div className="flex items-center justify-between pt-1.5 border-t border-amber-500/15 text-slate-400">
                ...
                <div className="flex flex-wrap items-center gap-2 pt-1">
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl text-xs font-extrabold flex items-center gap-1.5 transition shadow-md ${
                      isRunningLocally
                  ...
                  <button
                    ...
                    disabled={isSyncing}
                    className="px-3 py-2 rounded-xl bg-[#141724] hover:bg-[#1f2436] text-amber-200 text-xs font-semibold flex items-center gap-1.5 transition border border-amber-500/15"
                  ...
                  <button
                    ...
                    className="px-2.5 py-2 rounded-xl bg-[#141724] hover:bg-[#1f2436] text-amber-400 hover:text-amber-300 text-xs transition border border-amber-500/15"
                  ...
                  <button
                    ...
                    className="px-3 py-2 rounded-xl bg-[#141724] hover:bg-[#1f2436] text-amber-200 text-xs font-semibold flex items-center gap-1.5 transition ml-auto border border-amber-500/15"
                  >
      ...
      <div className="p-5 rounded-3xl bg-[#0e1118] border border-amber-500/20 space-y-4 shadow-xl">
        <div className="flex items-center justify-between border-b border-amber-500/15 pb-3">
          ...
          <span className="text-xs text-amber-400/70 font-mono">Live Webhook Feed</span>
        </div>
        ...
        {activities.length === 0 ? (
          ...
          <div className="space-y-3">
            {activities.map((act) => (
              <div key={act.id} className="flex items-start gap-3 p-3 rounded-2xl bg-[#08090d] border border-amber-500/15 text-xs font-mono">
                ...
                <div className="flex-1 space-y-0.5">
                  ...
                  {act.actorName && <span className="text-[10px] text-amber-300 block">Author: {act.actorName}</span>}
                </div>
              </div>
            ))}
          </div>
      ...
      {isInviteModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/85 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-[#0e1118] border border-amber-500/30 rounded-3xl p-6 max-w-md w-full space-y-4 shadow-2xl shadow-black animate-scale-in">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-bold text-white flex items-center gap-2">
            ...
            {sentInviteInfo ? (
              <div className="space-y-4 py-2">
                ...
                {sentInviteInfo.gmailComposeUrl && (
                  <div className="space-y-2">
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="w-full py-3 px-4 rounded-xl bg-gradient-to-r from-amber-400 via-yellow-500 to-amber-600 hover:from-amber-300 hover:via-yellow-400 hover:to-amber-500 text-black text-xs font-extrabold shadow-lg shadow-amber-500/25 flex items-center justify-center gap-2 transition"
                    >
                ...
                <div className="space-y-1.5 font-mono text-xs">
                  <label className="text-amber-200 font-sans text-xs block font-semibold">Direct Join Link:</label>
                  <div className="flex items-center gap-2 bg-[#08090d] p-2.5 rounded-xl border border-amber-500/20">
                    <input
                      type="text"
                      readOnly
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 rounded-lg bg-gradient-to-r from-amber-400 to-yellow-500 text-black font-extrabold text-[11px] shrink-0 transition flex items-center gap-1"
                    >
                ...
                <div className="flex justify-end gap-3 pt-2">
                  <button
                    ...
                    className="px-4 py-2 rounded-xl bg-[#141724] hover:bg-[#1e2336] text-slate-300 text-xs font-semibold"
                  >
                    Invite Another
                  </button>
                  <button
                    ...
                    className="px-4 py-2 rounded-xl bg-gradient-to-r from-amber-400 to-yellow-500 text-black text-xs font-extrabold shadow-lg shadow-amber-500/25"
                  >
                    Done
                  </button>
                </div>
              ...
              <form onSubmit={handleInviteSubmit} className="space-y-4">
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-amber-200">Email Address (Gmail / Corporate)</label>
                  <input
                    type="email"
                    value={inviteEmail}
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-amber-200">Role</label>
                  <select
                    value={inviteRole}
                ...
                <div className="flex justify-end gap-3 pt-2">
                  ...
                  <button
                    type="submit"
                    disabled={isInviting}
                    className="px-4 py-2 rounded-xl bg-gradient-to-r from-amber-400 via-yellow-500 to-amber-600 hover:from-amber-300 hover:via-yellow-400 hover:to-amber-500 text-black text-xs font-extrabold shadow-lg shadow-amber-500/25 flex items-center gap-1.5"
                  >
                    {isInviting ? (
                      <>
        ...
        <div className="fixed inset-0 z-50 bg-black/85 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-[#0e1118] border border-amber-500/30 rounded-3xl p-6 max-w-md w-full space-y-4 shadow-2xl shadow-black animate-scale-in">
            ...
            <form onSubmit={handleConnectRepoSubmit} className="space-y-4">
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-amber-200">GitHub Repository (owner/repo)</label>
                <input
                  type="text"
                  value={repoInput}
                  onChange={(e) => setRepoInput(e.target.value)}
                  placeholder="company/crm"
                  className="w-full bg-[#08090d] border border-amber-500/20 rounded-xl px-3.5 py-2.5 text-xs text-white font-mono placeholder-slate-500 focus:outline-none focus:border-amber-400"
                  required
                />
              </div>

              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-amber-200">Branch</label>
                <input
                  type="text"
                  value={branchInput}
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button
                  ...
                  className="px-4 py-2 rounded-xl bg-gradient-to-r from-amber-400 via-yellow-500 to-amber-600 hover:from-amber-300 hover:via-yellow-400 hover:to-amber-500 text-black text-xs font-extrabold shadow-lg shadow-amber-500/25"
                >
      ...
      {activeLogsModule && (
        <div className="fixed inset-0 z-50 bg-black/85 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-[#0e1118] border border-amber-500/30 rounded-3xl p-6 max-w-2xl w-full space-y-4 shadow-2xl shadow-black animate-scale-in">
            <div className="flex items-center justify-between border-b border-amber-500/15 pb-3">
              <h3 className="text-lg font-bold text-white flex items-center gap-2">
            ...
            <div className="space-y-4 max-h-[400px] overflow-y-auto pr-1">
              {logsList.length === 0 ? (
                <div className="p-4 rounded-xl bg-[#08090d] text-slate-500 text-xs font-mono text-center">
                  No build deployment logs recorded yet.
                </div>
              ) : (
                logsList.map((log) => (
                  <div key={log.id} className="p-4 rounded-2xl bg-[#08090d] border border-amber-500/15 space-y-2 font-mono text-xs">
                    <div className="flex items-center justify-between text-slate-400">
                      <span className="font-bold text-amber-300">Commit {log.commitSha.slice(0, 7)} ({log.author || 'Developer'})</span>
                    ...
                    <pre className="p-3 rounded-xl bg-black text-slate-300 text-[11px] whitespace-pre-wrap font-mono border border-amber-500/15 max-h-48 overflow-y-auto">
                    ...
                    <div className="flex items-center justify-between pt-1">
                      ...
                      {log.commitSha !== activeLogsModule.currentCommitSha && (
                        <button
                          ...
                          className="px-3 py-1 rounded-xl bg-amber-500/20 hover:bg-amber-500/30 text-amber-300 border border-amber-500/30 text-xs font-semibold flex items-center gap-1.5 transition"
                        >
            ...
            <div className="flex justify-end pt-2">
              <button
                onClick={() => setActiveLogsModule(null)}
                className="px-4 py-2 rounded-xl bg-[#141724] hover:bg-[#1e2336] text-slate-300 text-xs font-semibold border border-amber-500/15"
              >
                Close Logs
              </button>
            </div>
      ...
      {rollbackPm && (
        <div className="fixed inset-0 z-50 bg-black/85 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-[#0e1118] border border-amber-500/30 rounded-3xl p-6 max-w-xl w-full space-y-4 shadow-2xl shadow-black animate-scale-in">
            <div className="flex items-center justify-between border-b border-amber-500/15 pb-3">
              <h3 className="text-lg font-bold text-white flex items-center gap-2">
              ...
              {rollbackLogs.length === 0 ? (
                ...
                rollbackLogs.map((log) => {
                  ...
                  return (
                    <div
                      key={log.id}
                      className={`p-3.5 rounded-2xl border font-mono text-xs flex items-center justify-between gap-3 ${
                        isCurrent
                          ? 'bg-[#14120a] border-amber-500/40 text-white'
                          : 'bg-[#08090d] border-amber-500/15 text-slate-300'
                      }`}
                    >
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          <span className="font-bold text-yellow-400">Commit {log.commitSha.slice(0, 7)}</span>
                          <span className="text-[10px] text-amber-300 font-sans">by {log.author || 'Developer'}</span>
                          {isCurrent && (
                            <span className="px-2 py-0.5 rounded text-[10px] bg-gradient-to-r from-amber-400 to-yellow-500 text-black font-extrabold">
                              Current Active
                            </span>
                          )}
                        </div>
                      ...
                      {!isCurrent && (
                        <button
                          ...
                          className="px-3 py-1.5 rounded-xl bg-gradient-to-r from-amber-400 to-yellow-500 hover:from-amber-300 hover:to-yellow-400 text-black text-xs font-extrabold shrink-0 shadow-md shadow-amber-500/25 transition"
                        >
